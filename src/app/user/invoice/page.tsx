            ...
            {sortedInvoices && sortedInvoices?.length > 0 ? (
              sortedInvoices?.map((invoice, index) => (
                <motion.tr
                  ...
                  onClick={() =>
                    router.push(
                      `/user/trips/invoice?party=${encodeURIComponent(invoice.party_id)}&route=${encodeURIComponent(invoice.route)}&trips=${encodeURIComponent(JSON.stringify(invoice.trips))}&invoiceId=${encodeURIComponent(invoice._id)}`,
                    )
                  }
